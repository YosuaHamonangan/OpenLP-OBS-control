  ...
  updateServiceList: function () {
    $.getJSON("/api/service/list").then((data, status) => {
      ...
      data.results.items.forEach((item, i) => {
        ...
        var $item = $(
          `<div class="service-item${selected ? " selected" : ""}"/>`
        );
        $item.html(title).click(() => {
          $.get(`/api/service/set?data={"request": {"id": ${i}}}`);
        });
    ...
    $.getJSON("/api/controller/live/text").then((data, status) => {
      ...
      data.results.slides.forEach((slide, i) => {
        ...
        rows.forEach((rowText) => {
          ...
          var $row = $(
            `<div class="text-item${selected ? " selected" : ""}"/>`
          );
          $row.html(rowText).click(() => {
            this.currentRow = row;
            if (this.currentSlide !== i) {
              this.currentSlide = i;
              $.get(`/api/controller/live/set?data={"request": {"id": ${i}}}`);
            }
            this.updateTextList();
          });
